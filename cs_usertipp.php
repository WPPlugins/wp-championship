...
/*  Copyright 2006-2016  Hans Matzen  (email : webmaster at tuxlog.de)
...
if( ! function_exists('show_UserTippForm') ) {//make it pluggable
	function show_UserTippForm()
	{
		...
		if ($cs_floating_link > 0 and !is_admin() )
			$out .= cs_get_float_js();
		...
		if (isset($_GET['cs_stellv']) and intval($_GET["cs_stellv"]) > 0 and ! $cs_stellv_schalter) {
			$sql="select ID, stellvertreter, user_nicename from $cs_users inner join $wp_users on ID=userid where userid=".intval($_GET["cs_stellv"]).";";
			$r2 = $wpdb->get_row($sql);
		...
		if ( !$wpcs_demo>0) {
		  ...
		  if ( array_key_exists('update',$_POST) and $_POST['update'] == __("Änderungen speichern","wpcs")) {
		    ...
		    if ($r1->anz > 0) {
		      $psql0 = $wpdb->prepare("update $cs_users set mailservice= %d , stellvertreter=%d , mailreceipt=%d where userid=$uid;",
					$_POST['mailservice'], $_POST['stellvertreter'], $_POST['mailreceipt']);
		    } else {
		      $psql0 = $wpdb->prepare("insert into $cs_users values ($uid,0,%d,%d,%d,0,'0000-00-00 00:00:00',-1,'');",
					$_POST['mailservice'], $_POST['stellvertreter'], $_POST['mailreceipt']);
		    }
		    $r3 = $wpdb->query($psql0);
		    ...
		    if ( $blog_now <= $mr->mintime ) {
		      $sql0 = "update  $cs_users set champion= ".intval($_POST['champion']).",championtime='".$currtime."' where userid=$uid;";
		      $r2 = $wpdb->query($sql0);
		    } else if ($r0[0]->champion != intval($_POST['champion'])) {
		      $out .= __("Championtipp kann nicht mehr verändert werden.","wpcs")."<br />\n";
		    }
		    
		    // Tippgruppe speichern und auf zulaessigkeit pruefen
		    if ( $blog_now <= $mr->mintime ) {
		      $sql0 = "update  $cs_users set tippgroup= ".intval($_POST['tippgroup'])." where userid=$uid;";
		      $r2 = $wpdb->query($sql0);
		    } else if ($r0[0]->tippgroup != intval($_POST['tippgroup'])) {
		      $out .= __("Tippgruppe kann nicht mehr verändert werden.","wpcs")."<br />\n";
		    }
			...
			if ($cs_goalsum > -1 and $cs_goalsum_auto==0 and $mkey=="gt3_" and $_POST['gt2_'.$mid] != -1 and $_POST['gt1_'.$mid] != -1) {
			  if ( intval($_POST[$key]) < $cs_goalsum and intval($_POST[$key]) >= 0) {
			...
			if ( substr($key,0,4) == "gt1_" or substr($key,0,4)=="gt2_" or substr($key,0,4)=="gt3_") {
			  ...
			  if (! isset($_POST['gt3_'.$mid])) $_POST['gt3_'.$mid] = -1;
			  ...
			  $have_tipps[$mid]=(int) $_POST['gt1_'.$mid] . ":" . (int) $_POST['gt2_'.$mid];
			}
		      }
		...
		//
		if ($cs_floating_link and ! is_admin())
		...
		$tgroup_select_html="";
		$sql="select tgid, name from $cs_tippgroup order by name;";
		$results1 = $wpdb->get_results($sql);
		...
		// anzeigen wenn eine Punkekorrektur vorhanden ist
		if (isset($r0[0]->penalty) && intval($r0[0]->penalty) != 0)
				$out .= "<div><b>".__("Für Dich wurde eine Punktekorrktur hinterlegt.","wpcs") . " (" . intval($r0[0]->penalty) . " " . __("Punkte",'wpcs') . ")";
		...
		if (get_option("cs_use_tippgroup") > 0) {
			$out .='<tr><td style="text-align:center" colspan="2">'.__("Tippgruppe","wpcs").": ";
		...
		if (intval($cs_number_of_tippdays) > 0) {
			...
			$sql = "select max(spieltag) as a from $cs_match where date(matchtime) <= current_date()";
			$low_spieltag = $wpdb->get_var($sql);
			if ($low_spieltag == NULL)
				$low_spieltag = 0;
			
			$sql = "select spieltag as a from $cs_match where date(matchtime) >= current_date() group by spieltag limit 1 offset " . $cs_number_of_tippdays;
			$high_spieltag = ($wpdb->get_var($sql)==null?$high_spieltag:$wpdb->get_var($sql));
		}
	
		// match loop
		// hole match daten
		if (get_option("cs_modus")==1)
			$sql="select a.mid as mid,b.groupid as groupid,b.tid as tid1, b.name as team1,b.icon as icon1, c.tid as tid2, c.name as team2,c.icon as icon2,a.location as location,date_format(a.matchtime,'%d.%m<br />%H:%i') as matchtime,a.matchtime as origtime,a.result1 as result1,a.result2 as result2,a.winner as winner,a.round as round, a.spieltag as spieltag from $cs_match a inner join $cs_team b on a.tid1=b.tid inner join $cs_team c on a.tid2=c.tid where a.round in ('V','F') and date(a.matchtime) >= '$high_date_to_show' order by origtime;";
		else
			$sql="select a.mid as mid,b.groupid as groupid,b.tid as tid1, b.name as team1,b.icon as icon1, c.tid as tid2, c.name as team2,c.icon as icon2,a.location as location,date_format(a.matchtime,'%d.%m<br />%H:%i') as matchtime,a.matchtime as origtime,a.result1 as result1,a.result2 as result2,a.winner as winner,a.round as round, a.spieltag as spieltag from $cs_match a inner join $cs_team b on a.tid1=b.tid inner join $cs_team c on a.tid2=c.tid where a.round = 'V' and a.spieltag between $low_spieltag and $high_spieltag order by spieltag,origtime;";
		...
		foreach($results as $res) {
			...
			if ( get_option('cs_modus') == 1 and (!isset($cs_col_group) or !$cs_col_group)) {
				if (get_option('cs_hovertable') == 1 and $res->round == "V") {
					...
					$hlink = $ajaxurl . "?action=tooltip_groupstats&amp;groupid=".($res->round == "V" ? $res->groupid : $res->mid);
			...
			if ( $is_admin ) {
				...
				if (array_key_exists('rt1_'.$res->mid,$errlist)) {
					$errclass = " cs_inputerror ";
					$rt1_value = (int) $_POST['rt1_'.$res->mid]; // alten eingabewert anzeigen
				} else {
					$errclass="";
				...
				if (array_key_exists('rt2_'.$res->mid,$errlist)) {
					$errclass = " cs_inputerror ";
					$rt2_value = (int) $_POST['rt2_'.$res->mid]; // alten eingabewert anzeigen
				} else {
					$errclass="";
			...
			if ($cs_goalsum > 0 and $cs_goalsum_auto==0) {			
				$gt3_value = (int) $_POST['gt3_'.$res->mid];
				...
				if ($res->result2 != -1 or $blog_now > $match_start)
					$out .= "<td>". (int) $_POST['gt3_'.$res->mid]."</td>";
				else
