...
/*  Copyright 2006-2015  Hans Matzen  (email : webmaster at tuxlog.de)
...
if (!function_exists('cs_admin')) { //make it pluggable
	...
	function cs_admin() {
		...
		if (isset($_POST['deltipps'])) $action = "deltipps";
		else 
		if (isset($_POST['delresults'])) $action = "delresults";
		else 
		if (isset($_POST['deltables'])) $action = "deltables";
		else 
		if (isset($_POST['mailservice1'])) $action = "mailservice1";
		else 
		if (isset($_POST['newcalc1'])) $action = "newcalc1";
		// update options
		...
		if ($action == "update") {
			...
			if ($_POST['cs_group_teams'] == "" or $_POST['cs_pts_winner'] == "" or $_POST['cs_pts_looser'] == "" or $_POST['cs_pts_deuce'] == "" or $_POST['cs_pts_champ'] == "" or $_POST['cs_pts_tipp'] == "" or $_POST['cs_pts_tendency'] == "" or $_POST['cs_pts_supertipp'] == "" or $_POST['cs_pts_oneside'] == "" or $_POST['cs_goalsum'] == "" or $_POST['cs_pts_goalsum'] == "") $errflag = 1;
			// send a message about mandatory data
			
			if ($errflag == 1) admin_message(__('Bitte alle Felder füllen.', "wpcs"));
			// update settings
			...
			if ($errflag == 0 and $action == "update") {
				...
				foreach ($pnarr as $pname) {
					// this is the only text field
					
					if ($pname == "cs_xmlrpc_news") {
						update_option("cs_xmlrpc_news", sanitize_text_field($_POST['cs_xmlrpc_news']));
					}
					else { // all integers
		...
		if ($action == "deltables" and $_POST['deltables_ok'] == 1) {
			// Tabellen  entfernen
			$sql = "drop table $cs_users, $cs_match, $cs_team, $cs_tipp, $cs_tippgroup;";
			$wpdb->query($sql);
		...
		$out.= '<div style="text-align:right;padding-bottom:10px;"><a class="button-secondary thickbox" href="'.site_url().'/wp-admin/admin-ajax.php?action=wpc_import&amp;height=700&amp;width=550&amp;fn=match" >' . __("Datenimport", "wpcs") . '</a>&nbsp;&nbsp;&nbsp;' . "\n";
		$out.= '<a class="button-secondary thickbox" href="'.site_url().'/wp-admin/admin-ajax.php?action=wpc_export&amp;height=700&amp;width=550&amp;fn=match" >' . __("Datenexport", "wpcs") . '</a>&nbsp;&nbsp;&nbsp;</div>' . "\n";
		...
		for ($i = 1;$i < 13;$i++) {
			...
			if ($i == $cs_groups) $out.= ' selected="selected"';
			$out.= '>' . $i . '</option>';
		}
		$out.= '</select></td>' . "\n";
		...
		if ($cs_newuser_auto > 0) $out.= " checked='checked' ";
		$out.= '/></td></tr>';
		// points for tendency
		...
		if ($cs_stellv_schalter > 0) $out.= " checked='checked' ";
		$out.= '/></td></tr>';
		...
		if ($cs_modus == 1) $out.= ' selected="selected"';
		$out.= '>' . __("Standard", "wpcs") . '</option>';
		$out.= '<option value="2"';
		
		if ($cs_modus == 2) $out.= ' selected="selected"';
		...
		if ($cs_reminder > 0) $out.= " checked='checked' ";
		$out.= '/></td></tr>';
		...
		if ($cs_goalsum_auto > 0) $out.= " checked='checked' ";
		$out.= '/>' . "\n";
		...
		if ($cs_goalsum_equal > 0) $out.= " checked='checked' ";
		$out.= '/><br /></th>' . "\n";
		...
		if ($cs_rank_trend > 0) $out.= " checked='checked' ";
		$out.= '/></td></tr>' . "\n";
		...
		if ($cs_floating_link > 0) $out.= " checked='checked' ";
		$out.= '/></td></tr>' . "\n";
		...
		if ($cs_hovertable > 0) $out.= " checked='checked' ";
		$out.= '/></td></tr>' . "\n";
		
		// number of days to show in tipp dialog
		$out.= '<tr><td colspan="2">&nbsp;</td><th scope="row" ><label for="cs_number_of_tippdays">' . __('Anzahl der angezeigten Spieltage', "wpcs") . ':</label></th>' . "\n";
		...
		if ($cs_lock_round1 > 0) $out.= " checked='checked' ";
		$out.= '/></td></tr>' . "\n";
				
		// tippgroup switch
		$out.= '<tr><td colspan="2">&nbsp;</td><th scope="row" ><label for="cs_use_tippgroup">' . __('Verwende Tippgruppen:', "wpcs") . ':</label></th>' . "\n";
		$out.= '<td ><input name="cs_use_tippgroup" id="cs_use_tippgroup" type="checkbox" value="1"  ';
		if ($cs_use_tippgroup > 0) $out.= " checked='checked' ";
		$out.= '/></td></tr>' . "\n";
		...
		if ($cs_xmlrpc_alltipps > 0) $out.= " checked='checked' ";
		$out.= ' />';
		...
		if ($cs_xmlrpc_shortname > 0) $out.= " checked='checked' ";
		$out.= '/></th>' . "\n";
		$out.= '<td class="td-admin"><input name="cs_xmlrpc" id="cs_xmlrpc" type="checkbox" value="1"  ';
		
		if ($cs_xmlrpc > 0) $out.= " checked='checked' ";
		$out.= ' /></td>' . "\n";
