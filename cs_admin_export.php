...
/*  Copyright 2014-2015  Hans Matzen  (email : webmaster at tuxlog.de)
...
function wpc_export_cb() {
	...
	if (array_key_exists('dlmode', $_POST)) {
		$dlmode = esc_attr($_POST['dlmode']);
	}
	
	if (array_key_exists('dlmode', $_GET)) {
		$dlmode = esc_attr($_GET['dlmode']);
	}
	$exmode = "";
	
	if (array_key_exists('exmode', $_POST)) {
		$exmode = esc_attr($_POST['exmode']);
	}
	
	if (array_key_exists('exmode', $_GET)) {
		$exmode = esc_attr($_GET['exmode']);
	}
	$fnmode = "";
	
	if (array_key_exists('fnmode', $_POST)) {
		$fnmode = esc_attr($_POST['fnmode']);
	}
	
	if (array_key_exists('fnmode', $_GET)) {
		$fnmode = esc_attr($_GET['fnmode']);
	}
	...
	$out.= '<p class="submit">';
	$out.= '<input type="submit" name="startexport" id="startexport" value="' . __('Export starten', 'wpcs') . ' &raquo;" onclick="submit_this(\'export\')" />';
	$out.= "&nbsp;&nbsp;&nbsp;";
	$out.= '<input type="submit" name="cancelexport" id="cancelexport" value="' . __('Schliessen', 'wpcs') . '" onclick="tb_remove();" />';
	$out.= '</p>' . "\n";
