...
if( ! function_exists('cs_admin_finals') ) {//make it pluggable
	function cs_admin_finals()
	{
		...
		// get the teams
		if ( $_POST['winner1'] == -1 ) 
		  $team1=sanitize_text_field($_POST['fgroup1'].$_POST['fplace1']);
		else 
		  $team1=sanitize_text_field(($_POST['winner1']==1 ? 'W' : 'V').$_POST['matchid1']);
		
		if ( $_POST['winner2'] == -1 ) 
		  $team2=sanitize_text_field($_POST['fgroup2'].$_POST['fplace2']);
		else 
		  $team2=sanitize_text_field(($_POST['winner2']==1 ? 'W' : 'V').$_POST['matchid2']); 
		...
		if ( $errflag==0 and $action == "savenew" ) {
		  ...
		  $psql = $wpdb->prepare("insert into $cs_match values (0,'F',-1, %d, %d,'%s','%s',-1,-1,-1,%d,%d);",
					$tid1,$tid2,$_POST['location'],$_POST['matchtime'],$tid1,$tid2);
		...
		if ( $errflag==0 and $action == "update" ) {
		  ...
		  $psql = $wpdb->prepare("update $cs_match set tid1=%d, tid2=%d, location='%s', matchtime='%s', ptid1=%d, ptid2=%d where mid=%d;",
					$tid1,$tid2,$_POST['location'],$_POST['matchtime'],$tid1,$tid2,$_POST['mid']);
		...
		if ( $action == 'remove' ) {
		$psql= $wpdb->prepare("delete from $cs_match where mid=%d;",$_GET['mid']);
		...
		$psql= $wpdb->prepare("select * from  $cs_match where mid=%d",$_GET['mid']);
